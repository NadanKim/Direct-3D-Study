...
#define PARTICLE_NUM 55

enum Kind { LEFT = 1, RIGHT, UP, DOWN, RAND };
...
public:
	...
	void ResetObjects(const float& zPos);
...
public:
	CParticle();
	CParticle(CGameObject* pOriginObject, float fMoveLength);
	CParticle(CParticle&& other) noexcept;
	~CParticle();

	CGameObject				*m_pParticles;
	XMFLOAT3					m_xmf3OriginPos;
	float								m_fMoveLength;

	bool								m_bRender;

	void SetFlame(CGameObject* pOriginObject,  float fMoveLength);
	void Update();

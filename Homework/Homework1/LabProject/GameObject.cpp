...
void CGameObject::ResetObjects(const float& zPos) {
	static uniform_int_distribution<> caseDst(1, 9);
	static uniform_int_distribution<> colorDst(0, 255);
	static uniform_real_distribution<float>speedDst(0.001f, 0.1f);
	static uniform_real_distribution<float>normalDst(0.0f, 1.0f);
	static uniform_real_distribution<float>posDst(-17.0f, 17.0f);
	...
	case Kind::DOWN:
		...
		break;
	default:
		float zDir = (zPos < 0) ? 1.0f : -1.0f;
		SetColor(RGB(colorDst(dre), colorDst(dre), colorDst(dre)));
		SetMovingDirection(XMFLOAT3(normalDst(dre), normalDst(dre), zDir));
		break;
	}
	SetPosition(posDst(dre), posDst(dre), zPos);
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	if (m_fShootRate > EPSILON) m_fShootRate -= fTimeElapsed;
}
...
CParticle::CParticle() {
	m_pParticles = new CGameObject[PARTICLE_NUM];
	m_bRender = TRUE;
}

CParticle::CParticle(CGameObject* pOriginObject, float fMoveLength) {
	m_pParticles = new CGameObject[PARTICLE_NUM];

	SetFlame(pOriginObject, fMoveLength);

	m_bRender = TRUE;
}

CParticle::CParticle(CParticle&& other) noexcept {
	m_xmf3OriginPos = other.m_xmf3OriginPos;
	m_fMoveLength = other.m_fMoveLength;
	m_bRender = other.m_bRender;
	m_pParticles = other.m_pParticles;
	other.m_pParticles = nullptr;
}
...
void CParticle::SetFlame(CGameObject* pOriginObject, float fMoveLength ) {
	static default_random_engine dre((unsigned)time(NULL));
	static uniform_real_distribution<float> urd(-1.0f, 1.0f);

	m_xmf3OriginPos = pOriginObject->GetPosition();
	m_fMoveLength = fMoveLength;

	DWORD dwColor = pOriginObject->m_dwColor;
	XMFLOAT3 axis = pOriginObject->m_xmf3RotationAxis;

	CMesh* pParticleMesh = new CCubeMesh(1.0f, 1.0f, 1.0f);
	for (int i = 0; i < PARTICLE_NUM; ++i) {
		m_pParticles[i].SetMesh(pParticleMesh);

		m_pParticles[i].SetColor(dwColor);
		m_pParticles[i].SetPosition(m_xmf3OriginPos);

		m_pParticles[i].SetMovingDirection(XMFLOAT3(urd(dre), urd(dre), urd(dre)));
		m_pParticles[i].SetMovingSpeed(0.1f);
	}
}

void CParticle::Update() {
	for (int i = 0; i < PARTICLE_NUM; ++i) {
		m_pParticles[i].Animate();
	}

	XMFLOAT3 xmf3Length;
	XMStoreFloat3(&xmf3Length, 
		XMVector3Length(XMLoadFloat3(&m_pParticles[0].GetPosition()) - XMLoadFloat3(&m_xmf3OriginPos)));
	if (xmf3Length.x > m_fMoveLength) {
		m_bRender = FALSE;
	}
}


void CParticle::Render(HDC hDCFrameBuffer, CCamera *pCamera) {
	if (m_bRender) {
		for (int i = 0; i < PARTICLE_NUM; ++i) {
			m_pParticles[i].Render(hDCFrameBuffer, pCamera);
		}
	}
}
...
CWallMeshDiffused::CWallMeshDiffused(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, float fWidth, float fHeight, float fDepth, XMFLOAT4 xmf4StartColor, XMFLOAT4 xmf4EndColor)
	: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//벽 메쉬를 표현하기 위한 정점 데이터이다.
	CDiffusedVertex pVertices[8 * 3];
	int i = 0;

	// 왼쪽 벽 면
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(- fx, -fy, -fz), xmf4StartColor);
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, +fy, -fz), xmf4StartColor);
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, +fy, +fz), xmf4EndColor);
	...
	XMFLOAT4 xmf4Color) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//총알 메쉬를 표현하기 위한 정점 데이터이다.
	CDiffusedVertex pVertices[4 * 3];
	int i = 0;

	// 총알을 위한 사면체를 표현하기 위한 메쉬
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(0.0f, fy, 0.0f),xmf4Color);
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(fx, -fy, -fz), Vector4::Add(xmf4Color,
		XMFLOAT4(0.0f, 0.3f, 0.4f, 1.0f)));
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, -fy, -fz), xmf4Color);

	pVertices[i++] = CDiffusedVertex(XMFLOAT3(0.0f, fy, 0.0f), xmf4Color);
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, -fy, -fz), xmf4Color);
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(0.0f, -fy, +fz), Vector4::Add(xmf4Color,
		RANDOM_COLOR));

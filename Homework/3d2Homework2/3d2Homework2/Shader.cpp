...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSInstancingTextured", "vs_5_1",
		ppd3dShaderBlob));
}

void CBillBoardShader::CreateShaderVariables(ID3D12Device *pd3dDevice,
...
void CBillBoardShader::UpdateShaderVariables(ID3D12GraphicsCommandList
	*pd3dCommandList)
{
	pd3dCommandList->SetGraphicsRootShaderResourceView(7,
		m_pd3dcbGameObjects->GetGPUVirtualAddress());
	for (int j = 0; j < m_nObjects; j++)
	{
		XMStoreFloat4x4(&m_pcbMappedGameObjects[j].m_xmf4x4World,
...
void CBillBoardShader::AnimateObjects(float fTimeElapsed)
{
	for (int j = 0; j < m_nObjects; j++)
	{
		m_ppObjects[j]->Animate(m_pPlayerCam);
	}
}
...
D3D12_SHADER_BYTECODE CTreeBBShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSBBTreeTextured", "ps_5_1",
		ppd3dShaderBlob));
}

void CTreeBBShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;

	float fxPitch = 16.0f * 3.5f;
	float fzPitch = 16.0f * 3.5f;
...
#ifdef _DEBUG
	std::cout << "나무:" << m_nObjects << std::endl;
#endif // DEBUG

	CTexture *pTexture = new CTexture(3, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Trees/Tree12.dds", 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Trees/Tree13.dds", 1);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Trees/Tree20.dds", 2);
	...
	CPlaneMeshTextured *pPlaneMesh = new CPlaneMeshTextured(pd3dDevice, pd3dCommandList, 60.0f, 130.0f);

	m_ppObjects = new CGameObject*[m_nObjects];

	XMFLOAT3 xmf3RotateAxis, xmf3SurfaceNormal;
	CBillBoardObject *pTreeObject = NULL;
	for (int i = 0, x = 0; x < xObjects; x++)
	{
		for (int z = 0; z < zObjects; z++)
		{
			pTreeObject = new CBillBoardObject(1);
			...
			pTreeObject->SetPosition(xPosition, fHeight + 64.0f, zPosition);
			...
			pTreeObject->Rotate(&xmf3RotateAxis, XMConvertToDegrees(fAngle));
...
D3D12_SHADER_BYTECODE CGrassBBShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSBBGrassTextured", "ps_5_1",
		ppd3dShaderBlob));
}

void CGrassBBShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;

	float fxPitch = 7.0f * 3.5f;
	float fzPitch = 7.0f * 3.5f;

	float fTerrainWidth = pTerrain->GetWidth();
	float fTerrainLength = pTerrain->GetLength();

	int xObjects = int(fTerrainWidth / fxPitch);
	int zObjects = int(fTerrainLength / fzPitch) /2;
	m_nObjects = (xObjects * zObjects);

#ifdef _DEBUG
	std::cout << "풀:" << m_nObjects << std::endl;
#endif // DEBUG

	CTexture *pTexture = new CTexture(3, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Grasses/Grass05.dds", 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Grasses/Grass08.dds", 1);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Grasses/Grass14.dds", 2);
	...
	CPlaneMeshTextured *pPlaneMesh = new CPlaneMeshTextured(pd3dDevice, pd3dCommandList, 30.0f, 15.0f);
...
#endif
			float xPosition = x * fxPitch;
			float zPosition = z * fzPitch + 2770;
			float fHeight = pTerrain->GetHeight(xPosition, zPosition);
			pTreeObject->SetPosition(xPosition, fHeight + 7.0f, zPosition);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
CFlowerBBShader::CFlowerBBShader()
{
}
...
D3D12_SHADER_BYTECODE CFlowerBBShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSBBFlowerTextured", "ps_5_1",
		ppd3dShaderBlob));
}

void CFlowerBBShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, void *pContext)
{
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)pContext;

	float fxPitch = 11.0f * 3.5f;
	float fzPitch = 11.0f * 3.5f;
...
#ifdef _DEBUG
	std::cout << "꽃:" << m_nObjects << std::endl;
#endif // DEBUG

	CTexture *pTexture = new CTexture(3, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Flowers/Flower01.dds", 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Flowers/Flower02.dds", 1);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Flowers/Flower17.dds", 2);

	CreateCbvAndSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, m_nObjects, 6);
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, pTexture, 9, false);
	...
	CPlaneMeshTextured *pPlaneMesh = new CPlaneMeshTextured(pd3dDevice, pd3dCommandList, 20.0f, 20.0f);
	...
	for (int i = 0, x = 0; x < xObjects; x++)
	{
		for (int z = 0; z < zObjects; z++)
		{
			...
			float zPosition = z * fzPitch + 2770;
			float fHeight = pTerrain->GetHeight(xPosition, zPosition);
			pTreeObject->SetPosition(xPosition, fHeight + 9.5f, zPosition);

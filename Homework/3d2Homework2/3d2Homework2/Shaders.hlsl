...
StructuredBuffer<CB_GAMEOBJECT_INFO> gGameObjectInfos : register(t12);
#else
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
Texture2D gtxtTreeTextures[3] : register(t0);
Texture2D gtxtGrassTextures[3] : register(t3);
Texture2D gtxtFlowerTextures[3] : register(t6);
...
*/
	float4 cColor = gtxtTreeTextures[NonUniformResourceIndex(nPrimitiveID/2)].Sample(gWrapSamplerState, input.uv);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
Texture2D gtxtTerrainBaseTexture : register(t9);
Texture2D gtxtTerrainDetailTexture : register(t10);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
Texture2D gtxtSkyBox : register(t11);
...
#ifdef _WITH_CONSTANT_BUFFER_SYNTAX
	output.position = mul(mul(mul(float4(input.position, 1.0f), gGameObjectInfos[nInstanceID].mtxWorld), gcbCameraInfo.mtxView), gcbCameraInfo.mtxProjection);
#else
	output.position = mul(mul(mul(float4(input.position, 1.0f), gGameObjectInfos[nInstanceID].mtxWorld), gmtxView), gmtxProjection);
#endif
	output.uv = input.uv;
	output.texIdx = nInstanceID;
	return(output);
}

float4 PSBBTreeTextured(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = gtxtTreeTextures[NonUniformResourceIndex(input.texIdx%3)].Sample(gWrapSamplerState, input.uv);
	if (cColor.a <= 0.2f) discard;
	return(cColor);
}

float4 PSBBGrassTextured(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = gtxtGrassTextures[NonUniformResourceIndex(input.texIdx % 3)].Sample(gWrapSamplerState, input.uv);
	if (cColor.a <= 0.2f) discard;
	return(cColor);
}

float4 PSBBFlowerTextured(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = gtxtFlowerTextures[NonUniformResourceIndex(input.texIdx % 3)].Sample(gWrapSamplerState, input.uv);
	if (cColor.a <= 0.2f) discard;
	return(cColor);
}
...
{
	m_xmf3Right = XMFLOAT3(0.632291, 0.205915, -0.746865);
	m_xmf3Up = XMFLOAT3(-0.136343, 0.978561, 0.154368);
	m_xmf3Look = XMFLOAT3(0.762640, 0.004224, 0.646810);
	m_xmf3Position = XMFLOAT3(33.107517, 307.641541, 401.151184);
	...
	m_iNextMissile = 0;
	m_fReloadTime = COOLTIME_PLAYER_RELOAD;
}
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	DWORD nCurrentCameraMode = m_pCamera->GetMode();
	if (nCurrentCameraMode == THIRD_PERSON_CAMERA || nCurrentCameraMode == SPACESHIP_CAMERA)
...
bool CPlayer::CollisionTestWithTerrain(void *pContext)
{
...
CGameObject* CPlayer::GetMissile()
{
	int nextMissile = m_iNextMissile++;

	if (m_iNextMissile >= MAX_PLAYER_MISSILE) m_iNextMissile = 0;
...
void CPlayer::SetMissile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList,
	ID3D12RootSignature *pd3dGraphicsRootSignature, CGameObject* pHellfileMissileFrame)
{
	for (int i = 0; i < MAX_PLAYER_MISSILE; ++i)
	{
...
void CPlayer::SetShaderEffect(CShader* shader)
{
	// 플레이어 자체가 터질 때를 위한 이펙트 설정
	m_effect = shader->GetEffect();
	for (int i = 0; i < MAX_PLAYER_MISSILE; ++i)
	{
		// 플레이어가 보유한 미사일에 이펙트 설정

...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//	m_nStride = sizeof(CTexturedVertex);
	m_nStride = sizeof(CBump2TexturedVertex);
	m_nOffset = 0;
	m_nSlot = 0;
	...
	XMFLOAT3 *xmf3Position = new XMFLOAT3[m_nVertices];
	XMFLOAT3 *xmf3Tangents = new XMFLOAT3[m_nVertices];
	XMFLOAT2 *xmf2TexCoord = new XMFLOAT2[m_nVertices];

	CBump2TexturedVertex *pVertices = new CBump2TexturedVertex[m_nVertices];
	...
	for (int i = 0, a = 0, z = zStart; z < (zStart + nLength); z++, a++)
	{
		for (int x = xStart, b = 0; x < (xStart + nWidth); x++, i++, b++)
		{
			fHeight = OnGetHeight(x, z, pContext);
			xmf3Position[i] = pVertices[i].m_xmf3Position = XMFLOAT3((x*m_xmf3Scale.x), fHeight, (z*m_xmf3Scale.z));
			...
			xmf2TexCoord[i] = pVertices[i].m_xmf2TexCoord1 = XMFLOAT2(a * 0.25f,b * 0.25f);

			xmf3Tangents[i] = XMFLOAT3(0, 0, 0);
	...
	CalculateTriangleStripVertexTangent(xmf3Tangents, xmf3Position, m_nVertices, xmf2TexCoord, pnIndices, m_nIndices);

	for (int i = 0; i < m_nVertices; ++i)
	{
		pVertices[i].m_xmf3Tangent = xmf3Tangents[i];
	}
...
void CHeightMapGridMesh::CalculateTriangleStripVertexTangent(XMFLOAT3 *pxmf3Tangents, XMFLOAT3 *pxmf3Positions, UINT nVertices,
...
CTexturedRectMesh::CTexturedRectMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, 
	...
	float fxPosition, float fyPosition, float fzPosition) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(CTexturedVertex);
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
// x, y이고 카메라를 향하는 것을 가정한 생성자, 이펙트 처리를 위해 이미지가 몇개로 나뉘어 있는지 파악하기 위한 변수 추가
CTexturedRectMesh::CTexturedRectMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList,
	float fWidth, float fHeight, int uvXPart, int uvYPart) : CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = 6;
	m_nStride = sizeof(CTexturedVertex);
	m_nOffset = 0;
	...
	float u = 1.0f / uvXPart;	// 이미지 한 장 u크기
	float v = 1.0f / uvYPart;   // 이미지 한 장 v크기

	// 오른쪽 삼각형
	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx, +fy, 0.0f), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx, -fy, 0.0f), XMFLOAT2(u, v));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx, -fy, 0.0f), XMFLOAT2(0.0f, v));
	// 왼쪽 삼각형
	pVertices[i++] = CTexturedVertex(XMFLOAT3(-fx, +fy, 0.0f), XMFLOAT2(0.0f, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx, +fy, 0.0f), XMFLOAT2(u, 0.0f));
	pVertices[i++] = CTexturedVertex(XMFLOAT3(+fx, -fy, 0.0f), XMFLOAT2(u, v));

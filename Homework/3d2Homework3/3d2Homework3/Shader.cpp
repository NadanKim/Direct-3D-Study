...
D3D12_SHADER_BYTECODE CShader::CompileShaderFromFile(WCHAR *pszFileName, LPCSTR pszShaderName, LPCSTR pszShaderProfile, ID3DBlob **ppd3dShaderBlob)
{
	...
	if (FAILED(::D3DCompileFromFile(pszFileName, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, 
		pszShaderName, pszShaderProfile, nCompileFlags, 0, ppd3dShaderBlob, &pd3dErrorBlob)))
	{
		...
		if (ack)
		{
			char *pErrorString = new char[size];
			sprintf(pErrorString, (const char*)ack, size);
...
D3D12_INPUT_LAYOUT_DESC CBillBoardShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[2] = { "TEXTURENUM", 0, DXGI_FORMAT_R32_UINT, 0, 20, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VS_BillBoard", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CBillBoardShader::CreateGeometryShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "GS_BillBoard", "gs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CBillBoardShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob)
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PS_BillBoard", "ps_5_1", ppd3dShaderBlob));
}
...
void CBillBoardShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	m_pTexture = new CTexture(1, RESOURCE_TEXTURE2DARRAY, 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Grasses/AGrassNTrees.dds", 0);

	CreateCbvAndSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, 0, 1);
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, m_pTexture, 10, false);

	for (int i = 0; i < m_nVertices; ++i)
	{
		// 위치 지정 단, 절벽에는 생기지 않도록 한다.
		while (true)
		{
			x = rand() % 730; z = rand() % 730;
			if ((pTerrain->GetNormal(x, z).y > 0.95f)) break;
		}

		pVertices[i].m_nTextureNumber = rand() % 3;	// 종류 설정
		// 종류에 따라 크기 설정
		switch (pVertices[i].m_nTextureNumber)
		{
		case 0: // 풀 1번
			pVertices[i].m_mxf2Size.x = rand() % 5 +10;
			pVertices[i].m_mxf2Size.y = rand() % 5 + 5;
			break;
		case 1:	// 나무 1번
			pVertices[i].m_mxf2Size.x = rand() % 20 +30;
			pVertices[i].m_mxf2Size.y = rand() % 25 + 20;
			break;
		case 2: // 나무 2번
			pVertices[i].m_mxf2Size.x = rand() % 20 + 25;
			pVertices[i].m_mxf2Size.y = rand() % 25 + 40;
			break;
		}
		pVertices[i].m_xmf3Position.x = x;
		pVertices[i].m_xmf3Position.y = pTerrain->GetHeight(x, z) + pVertices[i].m_mxf2Size.y / 2;
		pVertices[i].m_xmf3Position.z = z;
	}
...
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSInstancingTextured", "ps_5_1",
		ppd3dShaderBlob));
}

D3D12_INPUT_LAYOUT_DESC CEffectShader::CreateInputLayout()
{
	return CTexturedShader::CreateInputLayout();
}

void CEffectShader::CreateShaderVariables(ID3D12Device *pd3dDevice,
	ID3D12GraphicsCommandList *pd3dCommandList)
{
	//인스턴스 정보를 저장할 정점 버퍼를 업로드 힙 유형으로 생성한다.
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL,
		sizeof(CB_EFFECT_INFO) * m_nObjects, D3D12_HEAP_TYPE_UPLOAD,
		D3D12_RESOURCE_STATE_GENERIC_READ, NULL);
	//정점 버퍼(업로드 힙)에 대한 포인터를 저장한다.
	m_pd3dcbGameObjects->Map(0, NULL, (void **)&m_pcbMappedEffectObjects);
}
...
void CEffectShader::UpdateShaderVariables(ID3D12GraphicsCommandList
	*pd3dCommandList)
{
	pd3dCommandList->SetGraphicsRootShaderResourceView(11,
		m_pd3dcbGameObjects->GetGPUVirtualAddress());
	for (int j = 0; j < m_nObjects; j++)
	{
		XMStoreFloat4x4(&m_pcbMappedEffectObjects[j].m_xmf4x4World,
			XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[j]->m_xmf4x4World)));
		XMStoreFloat2(&m_pcbMappedEffectObjects[j].m_xmf2UV,
			XMLoadFloat2(&m_ppObjects[j]->m_xmf2AddUV));
	}
}

void CEffectShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, void *pContext)
{
	m_nObjects = MAX_ENEMY_MISSILE * 9 + 8 + 9 + 1;

	CTexture *pTexture = new CTexture(1, RESOURCE_TEXTURE2D_ARRAY, 0);
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/Particle/Explode64.dds", 0);
	...
	pEffectMaterial->SetTexture(pTexture);
#endif

	CTexturedRectMesh *pRectMesh = new CTexturedRectMesh(pd3dDevice, pd3dCommandList, 100.0f, 100.0f, 8, 8);

	m_ppObjects = new CGameObject*[m_nObjects];

	CGameObject *pEffectObject = NULL;
	for (int i = 0; i < m_nObjects; ++i)
	{
		m_ppObjects[i] = new CGameObject();
#ifndef _WITH_BATCH_MATERIAL
		m_ppObjects[i]->SetMaterial(pEffectMaterial);
#endif
		m_ppObjects[i]->m_bActive = true;
		m_ppObjects[i]->SetPosition(0, -2000.0f, 0.0f);
...
void CEffectShader::AnimateObjects(float fTimeElapsed)
{
}
...
CGameObject* CEffectShader::GetEffect()
{
	static int curCount = 0;

	return m_ppObjects[curCount++];
}
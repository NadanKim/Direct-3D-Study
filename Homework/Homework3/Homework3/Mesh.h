...
public:
	...
	int CheckRayIntersection(XMFLOAT3& xmRayPosition, XMFLOAT3& xmRayDirection, float
		*pfNearHitDistance);
	XMFLOAT3 CheckRayIntersection(XMFLOAT3& xmf3RayOrigin, XMFLOAT3& xmf3RayDirection);
...
class CFragmentMeshDiffused : public CMesh
{
public:
	CFragmentMeshDiffused(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
		*pd3dCommandList, float fWidth = 5.0f, float fHeight = 5.0f, float fDepth = 5.0f,
		XMFLOAT4 xmf4Color = XMFLOAT4(0.3f, 0.49f, 0.1f, 0.0f));
	virtual ~CFragmentMeshDiffused();
};
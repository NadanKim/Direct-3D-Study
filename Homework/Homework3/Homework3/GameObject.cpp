...
void CEnemyObject::Animate(float fTimeElapsed, XMFLOAT3 *xmf3PlayerPos)
{
	...
	// 플레이어를 향하는 벡터
	XMFLOAT3 dir;
	XMStoreFloat3(&dir, XMLoadFloat3(xmf3PlayerPos) - XMLoadFloat3(&pos));
	// 플레이어와의 거리
	float fLength = Vector3::Length(dir);
	// 높이는 변하지 않게 한다.
	dir.y = 0.0f;
	// 탐색범위보다 멀리있으면 움직이지 않는다.
	if (fLength > m_fSearchRange)
	{
		m_bMove = false;
		m_xmf3RealVelocity = XMFLOAT3(0.0f, 0.0f, 0.0f);
		return;
	}

	XMFLOAT3 xmf3Up(0.0f, 1.0f, 0.0f);
	// 플레이어의 방향과 up벡터를 외적하여 회전축을 얻는다.
	XMStoreFloat3( &m_xmf3RotationAxis, XMVector3Cross(XMLoadFloat3(&dir), XMLoadFloat3(&xmf3Up)));
	m_xmf3RotationAxis = Vector3::Normalize(m_xmf3RotationAxis);

	// 구한 회전축을 로컬 좌표계로 변경한다.
	m_xmf3RotationAxis = Vector3::TransformNormal(Vector3::Normalize(m_xmf3RotationAxis),
		XMLoadFloat4x4(&Matrix4x4::Inverse(m_xmf4x4World)));

	// 회전축으로 회전 시킨다.
	Rotate(&Vector3::Normalize(m_xmf3RotationAxis), - m_fRotationSpeed * fTimeElapsed);

	// 플레이어 쪽으로 이동한다.
	Move(dir, m_fMoveSpeed * fTimeElapsed);

	// Velocity를 저장한다 (벽과의 충돌시 반사방향을 구하는 용도)
	m_xmf3RealVelocity = Vector3::ScalarProduct(dir, m_fMoveSpeed * fTimeElapsed, false);
...
void CEnemyObject::OnObjUpdateCallback()
{
	...
	CHeightMapTerrain *pTerrain = (CHeightMapTerrain *)m_pObjUpdatedContext;

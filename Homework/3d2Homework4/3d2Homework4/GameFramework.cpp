	...
	switch (nMessageID)
	{
		case WM_KEYUP:
			switch (wParam)
			{
				...
				// 플레이어 사망 후 재시작
				case 'R':
					if (!m_pPlayer->m_bActive) m_pPlayer->Reset();
					break;
				// 플레이어 미사일 발사하기
				case VK_SPACE:
					if(m_pPlayer->m_bActive) m_pScene->LaunchPlayerMissile();
					break;
				case VK_F10:
					break;
				default:
...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetSize(6.0f);
	
	m_pScene->SetPlayerMissile();
...
void CGameFramework::ProcessInput()
{
	...
	if (!bProcessedByScene && m_pPlayer->m_bActive)
	{
		DWORD dwDirection = 0;
		...
		if (pKeysBuffer['M'] & 0xF0) {
			XMFLOAT3 rit = m_pPlayer->GetRight();
	...
	if(m_pPlayer->m_bActive) m_pPlayer->Update(m_GameTimer.GetTimeElapsed());
}

void CGameFramework::AnimateObjects()
{
	float fTimeElapsed = m_GameTimer.GetTimeElapsed();
	if (m_pPlayer && m_pPlayer->m_bActive) m_pPlayer->Animate(fTimeElapsed);
	if (m_pScene) m_pScene->AnimateObjects(fTimeElapsed);
}

void CGameFramework::CollisionTests()
{
	if (m_pScene) m_pScene->CollisionTest();
}
...
{    
	...
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
#endif
	if (m_pPlayer->m_bActive) m_pPlayer->UpdateTransform(NULL);
	if (m_pPlayer->m_bActive) m_pPlayer->Render(m_pd3dCommandList, m_pCamera);
	...
	m_pd3dCommandList->SetGraphicsRootSignature(m_pScene->GetGraphicsRootSignature());
	m_pCamera->SetViewportsAndScissorRectsMiniMap(m_pd3dCommandList);

...
void CGameFramework::CreateSwapChain()
{
	...
	assert(SUCCEEDED(hResult) && " m_pdxgiFactory->CreateSwapChain Failed!! ");
	//스왑체인을 생성한다.

	hResult = m_pdxgiFactory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER);
	assert(SUCCEEDED(hResult) && " m_pdxgiFactory->MakeWindowAssociation Failed!! ");
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	HRESULT hResult = ::CreateDXGIFactory1(__uuidof(IDXGIFactory4), (void **)&m_pdxgiFactory);
	//DXGI 팩토리를 생성한다.
	...
	for (UINT i = 0; DXGI_ERROR_NOT_FOUND != m_pdxgiFactory->EnumAdapters1(i, &pd3dAdapter); i++)
	{
		...
		hResult = pd3dAdapter->GetDesc1(&dxgiAdapterDesc);
		assert(SUCCEEDED(hResult) && " pd3dAdapter->GetDesc1 Failed!! ");
	...
	if (!pd3dAdapter)
	{
		...
		assert(SUCCEEDED(hResult) && " m_pdxgiFactory->EnumWarpAdapter Failed!! ");
	...
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CheckFeatureSupport Failed!! ");
	m_nMsaa4xQualityLevels = d3dMsaaQualityLevels.NumQualityLevels;
	//디바이스가 지원하는 다중 샘플의 품질 수준을 확인한다.
	m_bMsaa4xEnable = (m_nMsaa4xQualityLevels > 1) ? true : false;
	//다중 샘플의 품질 수준이 1보다 크면 다중 샘플링을 활성화한다.

	// CreateFence(초기값, 플래그(기본은 none), uuid, address)
	hResult = m_pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence), (void
		**)&m_pd3dFence);
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CreateFence Failed!! ");
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CreateCommandQueue Failed!! ");
	//직접(Direct) 명령 큐를 생성한다.

	hResult = m_pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
		__uuidof(ID3D12CommandAllocator), (void **)&m_pd3dCommandAllocator);
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CreateCommandAllocator Failed!! ");
	...
	hResult = m_pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
		m_pd3dCommandAllocator, NULL, __uuidof(ID3D12GraphicsCommandList), (void
			**)&m_pd3dCommandList);
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CreateCommandList Failed!! ");
	//직접(Direct) 명령 리스트를 생성한다.

	hResult = m_pd3dCommandList->Close();
	assert(SUCCEEDED(hResult) && " m_pd3dCommandList->Close Failed!! ");
...
void CGameFramework::CreateRtvAndDsvDescriptorHeaps()
{
	...
	assert(SUCCEEDED(hResult) && " m_pd3dDevice->CreateDescriptorHeap Failed!! ");
...
void CGameFramework::CreateRenderTargetView()
{
	...
	for (UINT i = 0; i < m_nSwapChainBuffers; i++)
	{
		...
		assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->GetBuffer Failed!! ");
...
void CGameFramework::CreateDepthStencilView()
{
	...
	HRESULT hResult = m_pd3dDevice->CreateCommittedResource(&d3dHeapProperties, D3D12_HEAP_FLAG_NONE,
...
void CGameFramework::BuildObjects()
{
	...
	assert(SUCCEEDED(hResult) && " m_pd3dCommandList->Reset Failed!! ");
	...
	wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F9:
		{
			...
			// 여기에서 또 처리할 필요가 있나? SetFullscreenState 하면 알아서 WM_SIZE로 가는데??
			// OnResizeBackBuffers();
			break;
		}
		default:
...
void CGameFramework::WaitForGpuComplete()
{
	...
	assert(SUCCEEDED(hResult) && " m_pd3dCommandQueue->Signal Failed!! ");
	if (m_pd3dFence->GetCompletedValue() < nFenceValue)
	{
		//펜스의 현재 값이 설정한 값보다 작으면 펜스의 현재 값이 설정한 값이 될 때까지 기다린다.
		hResult = m_pd3dFence->SetEventOnCompletion(nFenceValue, m_hFenceEvent);
		assert(SUCCEEDED(hResult) && " m_pd3dFence->SetEventOnCompletion Failed!! ");
...
void CGameFramework::FrameAdvance()
{
	...
	assert(SUCCEEDED(hResult) && " m_pd3dCommandAllocator->Reset Failed!! ");
	...
	assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->Present Failed!! ");
...
void CGameFramework::OnResizeBackBuffers()
{
	...
	// 백 버퍼 크기가 바뀌게 되므로 Release 한뒤 다시 만들어 줘야 한다.
	...
	// 바탕화면 해상도가 그대로이고 백버퍼 크기만 변경
#ifdef _WITH_ONLY_RESIZE_BACKBUFFERS
	DXGI_SWAP_CHAIN_DESC dxgiSwapChainDesc;
	hResult = m_pdxgiSwapChain->GetDesc(&dxgiSwapChainDesc);
	assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->GetDesc Failed!! ");
	hResult = m_pdxgiSwapChain->ResizeBuffers(0, 0, 0, DXGI_FORMAT_UNKNOWN, 0);
	assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->ResizeBuffers Failed!! ");
	m_nSwapChainBufferIndex = 0;
#else // 바탕화면 해상도가 현재 창의 해상도로 변경
	DXGI_SWAP_CHAIN_DESC dxgiSwapChainDesc;
	hResult = m_pdxgiSwapChain->GetDesc(&dxgiSwapChainDesc);
	assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->GetDesc Failed!! ");
	hResult = m_pdxgiSwapChain->ResizeBuffers(m_nSwapChainBuffers, m_nWndClientWidth,
		m_nWndClientHeight, dxgiSwapChainDesc.BufferDesc.Format, dxgiSwapChainDesc.Flags);
	assert(SUCCEEDED(hResult) && " m_pdxgiSwapChain->ResizeBuffers Failed!! ");
	m_nSwapChainBufferIndex = 0;
#endif
	// 버퍼를 현재 화면 해상도에 맞게 다시 생성

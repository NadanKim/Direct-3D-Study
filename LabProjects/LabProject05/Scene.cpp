...
{
	//루트 시그너쳐를 생성한다.
	//어떤 리소스들이 그래픽스 파이프라인에 연결되는 가를 정의
...
#endif
	// 코드상에서 쉐이더 컴파일
	hResult = D3DCompileFromFile(L"Shaders.hlsl", NULL, NULL, "VSMain", "vs_5_1", nCompileFlags, 0, 
		&pd3dVertexShaderBlob, NULL);
	assert(SUCCEEDED(hResult) && " D3DCompileFromFile VSMain Failed!! ");
	hResult = D3DCompileFromFile(L"Shaders.hlsl", NULL, NULL, "PSMain", "ps_5_1", nCompileFlags, 0,
		&pd3dPixelShaderBlob, NULL);
	assert(SUCCEEDED(hResult) && " D3DCompileFromFile PSMain Failed!! ");
	...
	d3dRasterizerDesc.DepthBias = 0;	// 깊이값에 더해질 바이어스 값
	d3dRasterizerDesc.DepthBiasClamp = 0.0f;	// 픽셀의 깊이값에 더해질 최대값
	d3dRasterizerDesc.SlopeScaledDepthBias = 0.0f;	// 픽셀의 기울기에 따라 조절할 값
	d3dRasterizerDesc.DepthClipEnable = TRUE;
	d3dRasterizerDesc.MultisampleEnable = FALSE;
	d3dRasterizerDesc.AntialiasedLineEnable = FALSE;
	d3dRasterizerDesc.ForcedSampleCount = 0;	// UAV 렌더링에 사용할 샘플의 개수
